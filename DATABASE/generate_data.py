...
N_ORDER = 100000
N_ORDER_DETAIL = 150000

N_INVOICE = 100000
N_APPLY_DISCOUNT = 80000

N_BRANCH_SERVICE = 1000
N_BRANCH_PRODUCT = 15000

N_APPOINTMENT = 150000
N_REVIEW = 40000
N_ORDER_SERVICE = 100000
N_ORDER_S_DETAIL = 150000
...
def main():
    ...
    with open("Employee.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        for emp_id in employee_ids:
            ...
            if emp_id in doctor_emp_ids:
                ...
                role = random.choice(["Receptionist", "Cashier", "SalePerson"])

            employee_roles[emp_id] = role

            # Thêm vào sets theo role
            if role == "Receptionist":
                receptionist_ids.add(emp_id)
            elif role == "Cashier":
                cashier_ids.add(emp_id)
            elif role == "SalePerson":
                salesperson_ids.add(emp_id)
    ...
    with open("LeaveRequest.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        while count < N_LEAVE_REQUEST and attempts < max_attempts:
            ...
            request_date = (
                start_d_obj - timedelta(days=random.randint(1, 7))
            ).strftime("%Y-%m-%d")
            mid = pick(manager_ids) if manager_ids else emp
            w.writerow(
                [
    ...
    with open("Customer.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        for i in range(1, N_CUSTOMER + 1):
            ...
            if random.random() < 0.5:
                ...
                while uname in employee_accounts or uname in customer_accounts:
                    uname = f"{base}{suffix}"
                    suffix += 1
                username = uname
                customer_accounts[username] = fake_en.password(length=10)
    ...
    with open("Product.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        product_data = {
            "Thức ăn chó": [
                "Royal Canin Adult Chó",
                "Pedigree Hạt Khô",
                "Smartheart Chó Con",
                "Ganador Adult",
                "Classic Pets Chó",
                "Nutri Source Chó",
                "ANF Chó Trưởng Thành",
                "Taste of the Wild Chó",
                "Orijen Chó",
                "Acana Chó",
                "Wellness Chó",
                "Dog Mania Premium",
    ...
    # ---------- 18. VACCINATION PACKAGE ----------
    vacc_pkg_ids = []
    with open(
        "VaccinationPackage.csv", "w", newline="", encoding="utf-8-sig"
    ) as f:
        w = csv.writer(f)
        w.writerow(["VPID", "PetID", "Duration", "DiscountRate"])
    ...
    with open("Vaccination.csv", "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["VID", "PetID", "VaccineID", "VaccinationDate", "Dosage"])
        for sid in vacc_service_ids:
            vdate = random_datetime(
                datetime(2021, 1, 1), datetime(2024, 12, 31)
            )
            w.writerow([sid, pick(pet_ids), pick(vaccine_ids), vdate, "1ml/kg"])
        ...
        for sid in surgery_service_ids:
            ...
            w.writerow(
                [
                    sid,
                    pick(pet_ids),
                    pick(prescription_ids),  # PrescriptionID FK -> Prescription
                    pick(surg_status),
                    pick(surg_type),
                    pick(anesth),
                    sdate,
        ...
        for i in range(1, N_ORDER + 1):
            ...
            w.writerow(
                [
                    ...
                    pick(customer_ids),
                    (
                        pick(list(salesperson_ids))
                        if salesperson_ids
                        else pick(employee_ids)
                    ),
                    cdate,
                    random.choice(
    ...
    # ---------- 29. ORDER SERVICE ----------
    order_service_ids = []
    with open("OrderService.csv", "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "OrderServiceID",
        ...
        for i in range(1, N_ORDER_SERVICE + 1):
            osid = f"ORS{i:06d}"
            order_service_ids.append(osid)
            cdate = random_date(datetime(2022, 1, 1), datetime(2024, 12, 31))
            w.writerow(
                [
                    osid,
                    ...
                    cdate,
                    random.choice(
                        ["08:00:00", "10:30:00", "14:00:00", "16:00:00"]
                    ),
                    pick(branch_ids),
                ]
            )

    # ---------- 30. ORDER SERVICE DETAIL ----------
    used_order_s_detail = set()
    with open("OrderSDetail.csv", "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["OrderServiceID", "ServiceID"])
        while len(used_order_s_detail) < N_ORDER_S_DETAIL:
            osid = pick(order_service_ids)
            sid = pick(service_ids)
            key = (osid, sid)
            if key in used_order_s_detail:
                continue
            used_order_s_detail.add(key)
            w.writerow([osid, sid])
    ...
    with open("Invoice.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        for i in range(1, N_INVOICE + 1):
            ...
            w.writerow(
                [
                    iid,
                    pick(order_ids),  # OrderID FK -> Orders
                    pick(
                        order_service_ids
                    ),  # OrderServiceID FK -> OrderService
                    customer,
                    (
                        pick(list(cashier_ids))
                        if cashier_ids
                        else pick(employee_ids)
                    ),
                    card,
    ...
    with open("AccountLogin.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        for uname, pwd in sorted(employee_accounts.items()):
            w.writerow([uname, pwd])

    # ---------- 38.1 ACCOUNT LOGIN CUSTOMER ----------
    with open(
        "AccountLoginCustomer.csv", "w", newline="", encoding="utf-8-sig"
    ) as f:
        w = csv.writer(f)
        w.writerow(["Username", "Password"])
        for uname, pwd in sorted(customer_accounts.items()):
            w.writerow([uname, pwd])

    # ---------- 39. SURGERY CHECKLIST TEMPLATE ----------
    checklist_ids = []
    with open(
        "SurgeryChecklistTemplate.csv", "w", newline="", encoding="utf-8-sig"
    ) as f:
        w = csv.writer(f)
        w.writerow(["ChecklistID", "ServiceID", "BodyPart", "CheckItem"])

        # Template kiểm tra cho từng loại phẫu thuật
        body_parts = [
            "Vết mổ",
            "Thân nhiệt",
            "Toàn thân",
        ...
        check_items = {
            ...
            "Vùng bụng": [
                "Sờ nắn kiểm tra đau",
                "Kiểm tra chướng bụng",
            ],
            "Hô hấp": [
                "Đếm nhịp thở",
                "Kiểm tra tiếng thở",
                "Kiểm tra màu niêm mạc",
            ],
            "Tim mạch": [
                "Đo nhịp tim",
                "Kiểm tra mạch",
        ...
        idx = 1
        # Tạo checklist template cho các ca phẫu thuật
        for sid in surgery_ids:
            # Mỗi ca phẫu thuật có 3-6 hạng mục kiểm tra
            num_items = random.randint(3, 6)
            selected_parts = random.sample(
                body_parts, min(num_items, len(body_parts))
            )

            for part in selected_parts:
                item = pick(check_items[part])
                chk_id = f"CHK{idx:05d}"
                checklist_ids.append(chk_id)
                w.writerow([chk_id, sid, part, item])
                idx += 1

    # ---------- 40. NURSE ASSIGNMENT ----------
    assignment_ids = []
    with open(
        "NurseAssignment.csv", "w", newline="", encoding="utf-8-sig"
    ) as f:
        w = csv.writer(f)
        w.writerow(
            ["AssignmentID", "SurgeryID", "NurseID", "AssignedDate", "Note"]
        )

        nurse_list = list(nurse_emp_ids) or employee_ids
        notes = [
            "Theo dõi sát 24h đầu",
            "Ca khó - chú ý nhé",
            "Theo dõi hậu phẫu thường quy",
            "Bệnh nhân VIP - chăm sóc kỹ",
        ...
        used_assignment = set()
        idx = 1
        # Phân công y tá cho các ca phẫu thuật
        for sid in surgery_ids:
            # Mỗi ca có thể có 1-2 y tá phụ trách
            num_nurses = random.randint(1, 2)
            assigned_nurses = random.sample(
                nurse_list, min(num_nurses, len(nurse_list))
            )

            for nid in assigned_nurses:
                key = (sid, nid)
                if key in used_assignment:
                    continue
                used_assignment.add(key)

                asn_id = f"ASN{idx:05d}"
                assignment_ids.append(asn_id)

                # Ngày phân công là ngày phẫu thuật hoặc 1-2 ngày sau
                assigned_date = random_date(
                    datetime(2021, 1, 1), datetime(2024, 12, 31)
                )

                w.writerow([asn_id, sid, nid, assigned_date, pick(notes)])
                idx += 1

    # ---------- 41. NURSE TASK ----------
    with open("NurseTask.csv", "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "TaskID",
        ...
        for asn_id in assignment_ids:
            # Mỗi assignment có 3-5 task
            num_tasks = random.randint(3, 5)
            selected_tasks = random.sample(task_names, num_tasks)

            for task_name in selected_tasks:
                tsk_id = f"TSK{idx:05d}"

                # 70% task đã hoàn thành
                is_completed = 1 if random.random() < 0.7 else 0
                completed_time = ""
                if is_completed:
                    completed_time = random_datetime(
                        datetime(2021, 1, 1), datetime(2024, 12, 31)
                    )

                w.writerow(
                    [tsk_id, asn_id, task_name, is_completed, completed_time]
                )
                idx += 1
