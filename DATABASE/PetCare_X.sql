...
-- =========================
-- 1. ACCOUNT LOGIN CUSTOMER
-- =========================  
CREATE TABLE AccountLoginCustomer (
...
CREATE TABLE TransferHistory (
    ...
    StartDate  DATE,
    EndDate    DATE,
    PRIMARY KEY (BranchID, EmployeeID, StartDate),
    FOREIGN KEY (EmployeeID) REFERENCES Employee(EmployeeID),
...
CREATE TABLE Customer (
    ...
    Birthday     DATE,
    Username VARCHAR(20),   -- liên kết tới ACCOUNT_LOGIN
    FOREIGN KEY (Username) REFERENCES AccountLoginCustomer(Username)
);
GO
...
CREATE TABLE Examination (
    ...
    FollowUpDate    DATE,
    FOREIGN KEY (PetID) REFERENCES Pet(PetID),
	FOREIGN KEY (EID) REFERENCES Service(ServiceID)
);
GO
...
-- User-Defined Table Type for passing drug list to procedures
CREATE TYPE DrugListType AS TABLE
(
    DrugID           VARCHAR(20) NOT NULL,
    Quantity         INT NOT NULL CHECK (Quantity > 0),
    UsageInstruction NVARCHAR(255) NOT NULL
);
GO
...
CREATE TABLE Surgery (
    ...
    PetID          VARCHAR(20),
    PrescriptionID VARCHAR(20),
    SurgeryStatus  NVARCHAR(100) NOT NULL,
    SurgeryType    NVARCHAR(100) NOT NULL,
    AnesthesiaType NVARCHAR(100) NOT NULL,
    SurgeryDate    DATETIME NOT NULL,
    DiagnosisNote  NVARCHAR(200) NOT NULL,
    FOREIGN KEY (PetID)          REFERENCES Pet(PetID),
    FOREIGN KEY (PrescriptionID) REFERENCES Prescription(PrescriptionID),
...
CREATE TABLE Discount (
    ...
    Percentage   FLOAT,
    MID          VARCHAR(20),
	MinLevelID   VARCHAR(10),
    FOREIGN KEY (MID) REFERENCES Employee(EmployeeID),
	FOREIGN KEY (MinLevelID) REFERENCES MembershipLevel(LevelID),
);
GO
...
CREATE TABLE OrderService (
    OrderServiceID VARCHAR(20) PRIMARY KEY,
    CustomerID     VARCHAR(20),
	PetID		   VARCHAR(20),
    CreateDate     DATE,
    CreateTime     TIME,
...
CREATE TABLE OrderSDetail (
    OrderServiceID VARCHAR(20),
    ServiceID      VARCHAR(20),
    PRIMARY KEY (OrderServiceID, ServiceID),
    FOREIGN KEY (OrderServiceID) REFERENCES OrderService(OrderServiceID),
...
-- =========================
-- 20. NURSE ASSIGNMENT & TASK
-- =========================
CREATE TABLE SurgeryChecklistTemplate (
    ChecklistID VARCHAR(20) PRIMARY KEY,
    ServiceID   VARCHAR(20),
...
CREATE TABLE NurseTask (
    ...
    FOREIGN KEY (AssignmentID) REFERENCES NurseAssignment(AssignmentID)
);
GO
